...
<form method="POST" action="{{ route('staffs.update', $staff) }}" class="bg-white rounded-xl shadow-md p-4">
    @csrf
    @method('PUT')

    <div class="mb-3">
        <label class="form-label text-pink-700">Nama</label>
        <input type="text" name="name" class="form-control border-pink-300 shadow-sm" value="{{ old('name', $staff->name) }}" required>
    </div>

    <div class="mb-3">
        <label class="form-label text-pink-700">Spesialisasi</label>
        <input type="text" name="speciality" class="form-control border-pink-300 shadow-sm" value="{{ old('speciality', $staff->speciality) }}" required>
    </div>
